...
export default function OperationsGridPage() {
  ...
  const fetchGrid = useCallback(() => {
    ...
    apiClient
      ...
      .catch(() => {
        // High-fidelity fallback for offline / mock dev mode
        const letters = isServer ? [serverRoomLetter || 'B'] : ['A', 'B', 'C', 'D', 'E', 'F', 'G', 'H'];
        const mockRooms = letters.map((letter) => ({
          id: `room-${letter.toLowerCase()}`,
          letter,
          name: `Sector ${letter} — Core Operations`,
          assignedServers: [
            {
              id: `srv-${letter}-1`,
              name: `Server ${letter}1`,
              presenceState: 'IN' as const,
              currentLocation: `${letter}4`,
            },
            {
              id: `srv-${letter}-2`,
              name: `Server ${letter}2`,
              presenceState: 'IN' as const,
              currentLocation: `${letter}3`,
            },
            {
              id: `srv-${letter}-3`,
              name: `Server ${letter}3`,
              presenceState: 'OUT' as const,
              currentLocation: 'Outside',
            },
          ],
          serverPresenceCount: 2,
          serverTotalCount: 3,
          serverCoverageSummary: '2 / 3 Present',
          subrooms: Array.from({ length: 8 }, (_, idx) => {
            const num = idx + 1;
            const code = `${letter}${num}`;
            const isB4 = code === 'B4';
            return {
              id: `subroom-${letter.toLowerCase()}${num}`,
              code,
              number: num,
              roomLetter: letter,
              memberCapacity: 4,
              serverSeatCount: 1,
              occupancyCount: (num % 3) + 1,
              members: Array.from({ length: (num % 3) + 1 }, (_, mIdx) => ({
                id: `m-${code}-${mIdx}`,
                name: `Member ${code}-${mIdx + 1}`,
                role: 'MEMBER' as const,
                title: 'Operations Engineer',
                presenceState: 'IN' as const,
                presenceLabel: 'In Subroom',
                currentLocation: code,
                durationInWorkGrid: '5h 42m',
                lastSeenIST: '03:40 PM IST',
                activeTaskId: mIdx === 0 ? 'TSK-8421' : undefined,
              })),
              serversPresent:
                num === 4
                  ? [
                      {
                        id: `srv-${letter}-1`,
                        name: `Server ${letter}1`,
                        email: `server.${letter.toLowerCase()}1@workgrid.corp`,
                        assignedRoomLetter: letter,
                        presenceState: 'IN' as const,
                        currentLocation: `${letter}4`,
                  ...
                  ? [
                      {
                        id: `srv-${letter}-2`,
                        name: `Server ${letter}2`,
                        email: `server.${letter.toLowerCase()}2@workgrid.corp`,
                        assignedRoomLetter: letter,
                        presenceState: 'IN' as const,
                        currentLocation: `${letter}3`,
                        isCurrentlyInSubroom: true,
                  ...
                  : [],
              activeRoomEvent: isB4
                ? {
                    id: 'event-b4',
                    title: 'AI Infrastructure Workshop',
                    startTimeIST: '03:30 PM IST',
        ...
        setGridData({
          currentTimeIST: formatToISTTime(new Date()),
          activeCompanyEvent: {
            id: 'event-company-1',
            title: 'Annual Company All-Hands & Technical Briefing',
            description: 'Organization-wide strategy address and core infrastructure sync.',
            scope: 'COMPANY',
            locations: ['Main Auditorium', 'Sector B Briefing', 'Remote Link'],
            startTimeIST: '03:00 PM IST',
            endTimeIST: '05:00 PM IST',
            requiredServersCount: 18,
            serversPresentCount: 16,
            serverCoverageSummary: '16 / 18 Present',
          },
          totalRooms: mockRooms.length,
          totalSubrooms: mockRooms.length * 8,
          totalPeoplePresent: 98,
          totalServersPresent: mockRooms.length * 2,
          rooms: mockRooms,
        });
        setIsLoading(false);
      });
